...
class MeioUploadBehavior extends ModelBehavior {
	...
	function setup(&$model, $config=array()) {
		...
		foreach($config as $field => $options) {
			...
			if(empty($options['url'])) {
				$this->__fields[$field]['url'] = 'uploads' . DS . $model->name;
			// Else replace the tokens of the dir.
			} else {
				$this->__fields[$field]['url'] = $this->replaceTokens($options['url'],$field);
			}
			...
			if($options['url'][strlen($options['url'])-1] == DS) {
				$options['url'] = substr($options['url'],0,strlen($options['url'])-2);
			}
		}
	}
	...
	function replaceTokens($string,$fieldName){
		return str_replace(array('{model}', '{field}', '{DS}','/','\\','{jojo}'),array(Inflector::underscore($this->__model->name),$fieldName,DS,DS,DS,$this->__model->theURL),$string);
	}
	...
	function uploadCheckDir(&$model, $data){
		foreach($data as $fieldName => $field){
			if(!$this->__model->validate[$fieldName]['url']['check']) return true;
			$options = $this->__fields[$fieldName];
			if(empty($field['remove']) || empty($field['name'])){
				// Check if directory exists and create it if required
				if(!is_dir($options['url'])) {
					if($options['create_directory']){
						if(!$this->Folder->create($options['url'])) {
							trigger_error('UploadBehavior Error: The directory '.$options['url'].' does not exist and cannot be created.', E_USER_WARNING);
							return false;
						}
					} else {
						trigger_error('UploadBehavior Error: The directory'.$options['url'].' does not exist.', E_USER_WARNING);
						return false;
					}
				}
				
				// Check if directory is writable
				if(!is_writable($options['url'])) {
					trigger_error('UploadBehavior Error: The directory '.$options['url'].' isn\'t writable.', E_USER_WARNING);
					return false;
				}
			}
		}
	...
	function setFileToRemove($fieldName){
		...
		if(!empty($filename) && $filename != $this->__fields[$fieldName]['default']){
			$this->__filesToRemove[] = array(
				'url' => $this->__fields[$fieldName]['url'],
				'name' => $filename
			);
		}
	}
	...
	function fixName($fieldName){
		...
		while(file_exists($this->__fields[$fieldName]['url'].DS.$newFilename.'.'.$ext)){
	...
	function beforeSave(&$model) {
		foreach($this->__fields as $fieldName=>$options){
			...
			if(isset($model->theURL) && $model->theURL != "") {
				$options['url'] = $model->theURL;
			
				$this->__fields[$fieldName]['url'] = $this->replaceTokens($options['url'],$fieldName);
			
				if($options['url'][strlen($options['url'])-1] == DS) {
					$options['url'] = substr($options['url'],0,strlen($options['url'])-2);
				}
			
				$this->uploadCheckDir(&$model,$this->__fields);
			}
			...
			$saveAs = $options['url'].DS.$model->data[$model->name][$fieldName]['name'];
			...
			if (count($options['allowed_ext']) > 0 && in_array($model->data[$model->name][$fieldName]['type'], array('image/jpeg', 'image/pjpeg', 'image/png'))) {
				foreach ($options['thumbsizes'] as $key => $value) {
					...
					// Otherwise, set the thumb filename to thumb.$key.$filename.$ext
						...
						$thumbSaveAs = $options['url'].DS.'thumb.'.$key.'.'.$model->data[$model->name][$fieldName]['name'];
					}
			...
			// Update model data
			$model->data[$model->name][$options['fields']['url']] = $options['url'];
	...
	function afterSave(&$model) {
		foreach($this->__filesToRemove as $file){
			if($file['name'])
				$this->_deleteFiles($file['name'], $file['url']);			
		}
		...
		if(isset($model->data)) {
			foreach($this->__fields as $field=>$options) {
				...
				if($file && $file != $options['default'])
					$this->_deleteFiles($file, $options['url']);
			}
		}
		return true;
	}
	...
	function createthumb($name, $filename, $new_w, $new_h)
	{
	   ...
	   imagecopyresampled($dst_img, $src_img, ($new_w - $thumb_w)/2, ($new_h - $thumb_h)/2, 0, 0, $thumb_w, $thumb_h, $old_x, $old_y);
